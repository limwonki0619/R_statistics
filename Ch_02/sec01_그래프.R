...
# 1. 자료의 갯수를 센다.
# 2. 자료의 최대 / 최소값을 구한다
# 3. 몇 개의 구간으로(급의 수)나눌지 결정한다.
#   - 자료의 갯수나 분포에 따라 달라져야 한다.
#   - 각 구간에 5개 이상의 숫자가 들어가도록 하는 것이 좋다
#   - 너무 많은 구간을 나누지 않도록 한다. (일반적으로 5~15구간)

# 4. 구간의 폭(급의 폭)을 구한다.
#   - 구간폭 = (최대값 - 최소값)/구간수
#   - 되도록이면 정수, 짝수, 5의 배수를 사용한다.
...
# 히스토그램은 도수분포표를 시각적으로 한눈에 파악하기 위해 만드는 것 
# 히스토그램의 모양을 통해 수집한 데이터가 어떻게 생겼는지 한 눈에 볼 수 있다.
# 즉, 히스토그램을 그리면, 수집한 데이터가 정규분포를 이루고 있는지, 특정 구간에 빈도 몰려 있는
# 비대칭적인 분포를 이루고 있는지, 혹은 이상값이 있는지 등
# 수집한 데이터를 요약하여 전반적인 분포를 파악하고자 할때 쓰인다. 
