...
n <- length(height)           # 표본 수 
x_bar <- mean(height)         # 표본평균
u <- 1220                     # mu
sd <- sd(height)              # 표본표준편차 
SE <- sd/sqrt(n) # 표준오차
...
# 자유도가 14인 t-분포에서 검정통계량 0.8465에 대한 유의확률은 0.8465보다 클 확률로 R에서 다음과 같이 구할 수 있다.

1 - pt(T,df=14)      # 검정통계량 t에 대한 양측검정(디폴트) 유의확률 -> 유의확률*2 or 유의수준/2 와 비교해야함
(1 - pt(T,df=14))*2  # 양측검정에서 실제 유의확률 -> 아래 t.test의 유의확률과 같은 값 
...
# 따라서 p-value는 0.4115이고, 유의수준은 0.025이므로 p-value > α 이므로 귀무가설을 채택한다. 

# T-test ------------------------------------------------------------------------------------------------------------ 
# t.test(x, y=NULL, alternative = c("two.sided(양측)","less(좌측)","greater(우측)"), 
#           mu=0, paired=F, var.equal=F, conf.level=0.95, ...)

# alternative: 양측검정, 단측검정의 경우 부등호
# mu : 평균은 기본 0
# paired : F : 디폴트, T : 두 표본에서 짝을 이룬 두 표본인 경우
# var.equal : T :  두 표본의 분산이 동일할 경우
# conf.level : 신뢰수준(1-α) 0.9, 0.95, 0.99

t.test(height, mu=1220, alternative = "two.sided") # 양측검정(디폴트), α = 0.05, p-valeu 0.4115 -> H0 채택 
t.test(height, mu=1220, alternative = "less")      # 좌측검정, α = 0.05, H1 : 7세 남자아이의 키가 1220보다 작다 p-valeu 0.7942 -> H0 채택
t.test(height, mu=1220, alternative = "greater")   # 우측검정, α = 0.05, H1 : 7세 남자아이의 키가 1220보다 크다 p-valeu 0.2058 -> H0 채택
